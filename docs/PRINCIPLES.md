...
1. **Layout primitives** (`Box`, `Stack`, `Inline`, `Grid`, `Container`, `Separator`) — solve layout consistently. Each flow primitive owns exactly one axis: `Stack` is vertical, `Inline` is horizontal. Neither can be flipped, so `align` and `justify` always mean what they mean in flexbox.
...
- 2026-07-26 — **`Stack` is vertical-only and `Center` is deleted.** `Stack` had a `direction` prop, so the same `align`/`justify` values silently swapped visual axis depending on a prop that was often set far from the alignment that depended on it — the prior that "`align` is vertical, `justify` is horizontal" is right for `direction="row"` and inverted for the default `column`. The fix removes the flip trigger rather than renaming the props: with a fixed per-component axis, the standard flexbox reading is exactly correct everywhere, so humans and agents keep a prior they already have instead of learning Silk-specific axis names. `direction`, `wrap`, and `collapseBelow` leave `Stack` (`Inline` keeps all three); horizontal flow is `Inline`, whose wrapping default is the intrinsic-first behavior the responsive strategy already assumes. `Center` is deleted rather than redefined: `Container` already owns max-width measure centering (`margin-inline: auto` + `max-width`), and one-axis or two-axis content centering is now `Stack`/`Inline` `align`/`justify`, so a third spelling was pure redundancy — and its wrapper element broke `asChild` for every composite that used it. `Grid` gains `justify` so the layout layer is capability-symmetric; on `Grid` alone it maps to `justify-items`, because `columns` always produces `1fr` tracks and leaves nothing for `justify-content` to distribute.
...
- 2026-07-26 — **`measure` is a typography token, and content is not whitespace.** Added `semantic.measure` (a character count, delivered as `--silk-measure-*` in `ch`) with a `measure` axis on `Text`, and moved Avatar sizing off the space scale onto fixed px in `mediaScale`. Reason: line length is a property of the type, not of the layout column, so `Container`'s widths were the wrong home and a per-composite `max-width` hook was heading for one narrow decision per composite. The Avatar change is the same principle read against `density`: density remaps whitespace, so anything on the space scale shrinks with it — correct for gaps, wrong for a face, which is content. A count rather than a length keeps both platform-neutral.
- 2026-07-26 — **`size` on a media-bearing composite scales the whole unit.** One `mediaScale` table now supplies the media edge, the media-to-text gap, and the name/meta type roles to Identity, MediaObject, Comment, and Notification. Reason: four composites were choosing those independently and had already disagreed — two different gaps for the identical 40px case — which is the "many narrow decisions" the concept budget exists to prevent. Also: a composite must not hardcode an inset that the recipe default already owns, or siblings in a column rag against each other.
