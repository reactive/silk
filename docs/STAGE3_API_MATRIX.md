...
Escape hatches on every rendered part: `className`, `style`, React 19 `ref`, data attributes, and `asChild` where Radix supports it. Parts that assemble sub-parts around their children omit `asChild` from their props — see [ARCHITECTURE.md](ARCHITECTURE.md#aschild-with-decorations). Remaining Radix behavior props pass through as advanced escape hatches; Silk documents the controlled contract below.
...
| **Select** | `Root Trigger Value Content Group Label Item Separator` | `value`, `onValueChange`, `open`, `onOpenChange` | `size`, `density`; `Select` | Variant context on wrapped `Root` | Assembled: Trigger includes Icon; Content includes Viewport + scroll buttons; Item includes ItemText + indicator. `Content` and `Item` omit `asChild`. Defaults `position="popper"`. `Content.container`. Web. |
