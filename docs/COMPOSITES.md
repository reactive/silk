...
- **Threading:** replies nest inside the parent comment's content column, so every level renders its own `rail="start"` and indents by the media column with no depth arithmetic. Compact Comment (`size="sm"`, one-line byline, ghost/compact ActionBar); list bullets reset.
