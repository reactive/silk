...
- **Spacing:** internal `space-3`, section `space-4`, inter-card `space-5`. Card inset is the `cardRecipe` default — a composite that hardcodes its own inset rags against its siblings in a column. Density is inherited — set `density="compact"` explicitly when a nested section should compact.
- **Measure:** body copy that can grow wide takes `measure="prose"` on `Text` rather than a hand-set width. Layout width and reading width are different concerns — `Container size` is the former.
- **Boolean fields:** a checkbox or radio uses `Field.Root orientation="horizontal"`, which puts the control beside its label and aligns the description under the label. Do not hand-assemble the row; a settings row that pushes the control to the far edge is a separate layout choice and stays an explicit `Inline justify="between"`.
- **Media scale:** anything pairing leading media with a text column takes `size` and reads `mediaScale` for the avatar edge, the media-to-text gap, and the name/meta roles. Avatars are fixed px, not space steps: `density` rescales whitespace, and a face is content.
- **Typography:** author = `mediaScale[size].primaryRole` (`headingSm` at md/lg — a name is not subordinate to body — `label` at sm), body = `body`, meta/timestamps = `mediaScale[size].metaRole` + `textSecondary`, stat values scale with `size` (`headingSm` / `heading` / `headingLg`). Posts render `<article>`; post timestamps sit on the identity meta line (`@handle · Jul 26, 11:00 AM`); comments use a single byline (`Name · @handle · 11:00 AM`) with time-only stamps; threads render unstyled lists. Post engagement is a caption summary (`128 likes · 14 replies`), not a competing StatGroup dashboard.
...
- **Threading:** replies nest inside the parent comment's content column, so every level renders its own `rail="start"` and indents by the media column with no depth arithmetic. The rail contains replies only: a “continue thread” affordance belongs to the comment it continues, so it takes Comment's `footer` slot below the rail. Compact Comment (`size="sm"`, one-line byline, ghost/compact ActionBar); list bullets reset.
